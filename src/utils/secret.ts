...
import type { MoodleSavedConfig } from "./config.ts";

const SECRET_SERVICE = "tui-moodle";
...
export function buildSecretAccountKey(config: MoodleSavedConfig): string {
  return `${config.baseUrl}|${config.username}|${config.service}`;
}
...
function getWindowsSecretFilePath(): string {
  return join(getAppConfigDir(), "secrets.json");
}
...
function readWindowsSecretFile(): SecretFileData {
  const path = getWindowsSecretFilePath();
  if (!existsSync(path)) return { entries: {} };

  try {
    const parsed = JSON.parse(readFileSync(path, "utf-8")) as SecretFileData;
...
function writeWindowsSecretFile(data: SecretFileData): void {
  ensureConfigDir();
  writeFileSync(getWindowsSecretFilePath(), JSON.stringify(data, null, 2), { mode: 0o600 });
}

function encryptDpapi(plaintext: string): string {
  return runPowerShell(
    "Add-Type -AssemblyName System.Security;$bytes=[System.Text.Encoding]::UTF8.GetBytes($env:TUI_MOODLE_SECRET);$enc=[System.Security.Cryptography.ProtectedData]::Protect($bytes,$null,[System.Security.Cryptography.DataProtectionScope]::CurrentUser);[Convert]::ToBase64String($enc)",
    { TUI_MOODLE_SECRET: plaintext },
  );
}

function decryptDpapi(ciphertextBase64: string): string {
  return runPowerShell(
    "Add-Type -AssemblyName System.Security;$bytes=[Convert]::FromBase64String($env:TUI_MOODLE_SECRET_B64);$dec=[System.Security.Cryptography.ProtectedData]::Unprotect($bytes,$null,[System.Security.Cryptography.DataProtectionScope]::CurrentUser);[System.Text.Encoding]::UTF8.GetString($dec)",
    { TUI_MOODLE_SECRET_B64: ciphertextBase64 },
  );
}
...
export function getSecureStorageDiagnostic(): SecretStorageDiagnostic {
  ...
  if (process.platform === "win32") {
    const hasShell = ["powershell.exe", "powershell", "pwsh.exe", "pwsh"].some((candidate) =>
...
export async function savePassword(config: MoodleSavedConfig, password: string): Promise<void> {
  const accountKey = buildSecretAccountKey(config);
  ...
  if (process.platform === "linux") {
    runCommand(
      "secret-tool",
      ["store", "--label", "tui-moodle", "service", SECRET_SERVICE, "account", accountKey],
      { input: password },
    );
    return;
  }
...
export async function loadPassword(config: MoodleSavedConfig): Promise<string | null> {
  const accountKey = buildSecretAccountKey(config);
...
export async function clearPassword(config: MoodleSavedConfig): Promise<void> {
  const accountKey = buildSecretAccountKey(config);
