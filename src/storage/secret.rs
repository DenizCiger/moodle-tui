...
fn decrypt_dpapi(ciphertext_b64: &str) -> Result<String, StorageError> {
    run_powershell(
        ...
        &[("TUI_MOODLE_SECRET_B64", ciphertext_b64)],
    )
}
...
pub fn save_password(config: &SavedConfig, password: &str) -> Result<(), StorageError> {
    ...
    if cfg!(target_os = "macos") {
        run_command(
            "security",
            &["add-generic-password", "-a", &key, "-s", SECRET_SERVICE, "-w", password, "-U"],
            None,
        )?;
        return Ok(());
    }
    if cfg!(target_os = "linux") {
        run_command(
            "secret-tool",
            &["store", "--label", "tui-moodle", "service", SECRET_SERVICE, "account", &key],
            Some(password),
        )?;
        return Ok(());
    }
...
pub fn load_password(config: &SavedConfig) -> Result<Option<String>, StorageError> {
    ...
    if cfg!(windows) {
        ...
        let encrypted = match store.entries.get(&key) {
